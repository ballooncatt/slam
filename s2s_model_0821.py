...
class slam_model_s2s(slam_model):
    ...
    def forward(self,
        ...
        def forward_process_embeds(input_embeds, labels, eps=1e-3):
            ...
            padid = torch.tensor([[[151666], [151666], [151666], [151666]]],device=input_ids.device)
        ...
        model_outputs = self.llm(inputs_embeds=noisy_embeds, attention_mask=None, labels=None)    # here we use the text token layer as the target label
        ...
        if total_loss == 0:
            ...
            print('masked_indices.sum()', masked_indices.sum())
        ...
        if self.metric:
            with torch.no_grad():
                preds = torch.argmax(xt, -1)  # shape: b, l
                masked_preds = preds[masked_indices]  # 只保留 masked 部分
                masked_labels = text_labels[masked_indices]  # 只保留 masked 部分
                text_acc = compute_accuracy(masked_preds.detach(), masked_labels.detach(), ignore_label=-100)
                
                if self.train_config.task_type != "asr":
                    preds_audio = [torch.argmax(xa[i], -1) for i in range(self.code_layer)]  # shape: b, l
                    audio_acc = [
                        compute_accuracy(
                            preds_audio[i][masked_indices].detach(),
                            audio_labels[:, i, :][masked_indices],
                            ignore_label=-100
                        )
                        for i in range(self.code_layer)
                    ]
    ...
    def generate_diffusion(self,
        ...
        pad_masked_embed = masked_embed.expand(batch_size, pad_len, embed_dim)

        # 拼接
        padded_input_embeds = torch.cat([inputs_embeds, pad_masked_embed], dim=1)



        # 初始化生成序列（文本和音频都填充为 mask）
        ...
        timesteps = torch.linspace(1, eps, steps + 1, device=padded_input_embeds.device)

        # 初始化生成序列
        generated_ids = [torch.full((pad_len,), mask_token_id, dtype=torch.long, device=padded_input_embeds.device) for _ in range(self.code_layer + 1)]
        # generated_ids[0~code_layer-1]: audio layers
        # generated_ids[code_layer]: text layer
        print('max_new_tokens',max_new_tokens)
        print('seq_len',seq_len)

        print('generated_ids',generated_ids[0].shape)

        # 迭代生成
        for step in range(steps):

            masked_indices = self.get_masked_indices_from_embeds(padded_input_embeds, masked_embed)
            masked_indices = masked_indices[0]  # [total_len]
            masked_indices = masked_indices[seq_len:]  # [pad_len]，布尔mask
            active_indices = masked_indices.nonzero(as_tuple=True)[0]  # 实际需要采样的位置，范围在[0, pad_len)

            outputs = self.llm(
                inputs_embeds=padded_input_embeds,                  # [btz, seq_len / 1, emb_dim]
                attention_mask=None,                
            )
            ...
            if alg == 'origin':
                ...
                # 采样mask
                x_t = generated_ids[self.code_layer][active_indices].clone()  # [num_active]
                transfer_index = torch.rand_like(x_t, dtype=torch.float) < p_transfer  # [num_active]布尔
                if transfer_index.any():
                    _, x_t[transfer_index] = sample_tokens(
                        xt_logits[seq_len + active_indices][transfer_index], temperature=temperature, top_p=top_p, top_k=top_k
                    )
                update_indices = active_indices[transfer_index]  # 需要写回的位置
                generated_ids[self.code_layer][update_indices] = x_t[transfer_index]

                
                # 采样音频token，每层独立采样
                for i in range(self.code_layer):
                    x_a_i = generated_ids[i][active_indices].clone()
                    if transfer_index.any():
                        _, x_a_i[transfer_index] = sample_tokens(
                            xa_logits[i][seq_len + active_indices][transfer_index], temperature=temperature, top_p=top_p, top_k=top_k
                        )
                    generated_ids[i][update_indices] = x_a_i[transfer_index]
                    # print("audio_input_ids min:", generated_ids[i][update_indices].min().item())
                    # print("audio_input_ids max:", generated_ids[i][update_indices].max().item())

                
                # print("text_input_ids min:", generated_ids[self.code_layer][transfer_index].min().item())
                # print("text_input_ids max:", generated_ids[self.code_layer][transfer_index].max().item())
                

                # 拼接成最终token id序列
                # 只对active_indices做layershift和拼接
                audio_tokens = torch.cat([
                    layershift(generated_ids[i][update_indices], i).unsqueeze(1)
                    for i in range(self.code_layer)
                ], dim=1)  # [num_update, code_layer]
                text_tokens = generated_ids[self.code_layer][update_indices].unsqueeze(1)  # [num_update, 1]
                combined_input_ids = torch.cat([audio_tokens, text_tokens], dim=1)

                # print("combined_input_ids min:", combined_input_ids.min().item())
                # print("combined_input_ids max:", combined_input_ids.max().item())

                # return None

                # 编码为embedding
                if hasattr(self.llm.model, "embed_tokens"):
                    new_embeds = self.llm.model.embed_tokens(combined_input_ids)
                elif hasattr(self.llm.model.model, "embed_tokens"):
                    new_embeds = self.llm.model.model.embed_tokens(combined_input_ids)
                else:
                    new_embeds = self.llm.model.model.model.embed_tokens(combined_input_ids)

                new_embeds = torch.mean(new_embeds, dim=1)  # [num_active, emb_dim]

                # 更新对应位置的 embedding
                padded_input_embeds[0, seq_len + update_indices, :] = new_embeds.clone()

        # 文本层 eot 截断 
        text_tokens = generated_ids[self.code_layer]
        if (text_tokens == eot).any():
            end_pos = (text_tokens == eot).nonzero(as_tuple=True)[0][0]
            generated_ids[self.code_layer] = text_tokens[:end_pos]
        else:
            generated_ids[self.code_layer] = text_tokens

        if do_layershift and (generated_ids[self.code_layer - 1] == eoa).any():
            end_ids = (generated_ids[self.code_layer - 1] == eoa).nonzero(as_tuple=True)[0][0]
            for i in range(self.code_layer):
                generated_ids[i] = generated_ids[i][:end_ids]
