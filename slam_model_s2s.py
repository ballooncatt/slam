...
class slam_model_s2s(slam_model):
    ...
    def forward(self,
        ...
        if self.metric:
            with torch.no_grad():
                ...
                text_acc = compute_accuracy(preds.detach(), text_labels.detach(), ignore_label=-100)

                if self.train_config.task_type != "asr":
                    preds_audio = [torch.argmax(xa[i], -1) for i in range(self.code_layer)]
                    audio_acc = [compute_accuracy(preds_audio[i].detach(), audio_labels[:,i,:], ignore_label=-100) for i in range(self.code_layer)]
                else:
                    audio_acc = [-1 for _ in range(self.code_layer)]
    ...
    def compute_parallel_loss_2(
        ...
        for i in range(self.code_layer):
            if audio_labels[:,i] is not None and self.train_config.task_type != "asr":
                single_audio_loss = F.cross_entropy(xa[i].reshape(-1, audio_vocab_size), audio_labels.reshape(-1), ignore_index=-100)
